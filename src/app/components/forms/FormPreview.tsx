    ...
    if (isEmpty(errors)) {
      try {
        const content: FormResponseContent = form.content.map((item) => {
          const choiceValue = choiceValues.find(
            (value) => value.id === item.id
          );

          if (choiceValue !== undefined) {
            return {
              element: omitObjProperty(item, [
                'description',
                'showDescription',
              ]),
      ...
      {isFormClosed && (
        <Center sx={{ height: '80vh' }}>
          <EmptyState
            ...
            subtitle="The form can't receive new submissions at this moment."
            icon={<IconEyeOff size="50px" />}
          />
        </Center>
      )}
                    ...
                    if (element.subtype === 'number') {
                      return (
                        <TextInput
                          ...
                          label={element?.label}
                          description={
                            element.showDescription && element?.description
                          }
                          required={element?.required}
                          disabled={isPreview}
                          {...register(`${element.id}`, {
