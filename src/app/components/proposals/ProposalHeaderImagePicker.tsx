...
import { CreateProposal, Proposal } from '@/core/types';
...
interface Props extends ModalProps {
  isLoading?: boolean;
  submit?: (data: CreateProposal) => Promise<Proposal>;
  proposal?: Proposal;
}

export default function ProposalHeaderImagePicker({
  opened,
  onClose,
  submit,
  proposal,
  ...
  const handleFileUpload = async (files: File[]) => {
    try {
      const url = await uploadFile(files[0], `${proposal?.userId}/images`);
      if (url) {
        await submit({ headerImgUrl: url });
        handleFormClose();
      }
    } catch (error) {
      console.log(error);
